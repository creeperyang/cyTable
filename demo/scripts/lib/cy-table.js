...
;(function(angular, factory) {
    ...
    */
    ...
    app.factory('cyTableParams', ['$q', '$log', '$filter', function($q, $log, $filter) {
        ...
        var cyTableParams = function(initState, initProps) {
                ...
                if (length === 2) {
                    if (changedStates.page && changedStates.count) {
                        return changedStates.page.cur === changedStates.page.prev &&
                            changedStates.count.cur <= changedStates.count.prev ? 'count-small' : false;
                    }
                }
                return false;
            };
            ...
            this.reload = function() {
                ...
                console.log('reload------', self.changedStates);

                log('cyTable: about to reload.');

                if ((self.cacheType = self.shouldUseCache(state, self.changedStates))) {
                    console.log('now use cache')
                    $defer.resolve({
                        list: self.originalData || self.data,//(self.sorting() ? $filter('orderBy')(self.data, self.orderBy()) :self.data).slice(0, self.count())
    ...
    app.run(['$templateCache', function($templateCache) {
        $templateCache.put('cy-table/filters/select-multiple.html', '<select ng-options="data.id as data.title for data in column.data" multiple ng-multiple="true" ng-model="params.filter()[name]" ng-show="filter==\'select-multiple\'" class="filter filter-select-multiple form-control" name="{{column.filterName}}"> </select>');
        $templateCache.put('cy-table/filters/select.html', '<select ng-options="data.id as data.title for data in column.data" ng-model="params.filter()[name]" ng-show="filter==\'select\'" class="filter filter-select form-control" name="{{column.filterName}}"> </select>');
